/**
 * Classe représentant l'ennemi Tourelle - astroport avec tourelle au centre
 */
class Tourelle {
    constructor(scene, x, y) {
        this.scene = scene;
        
        // Créer le sprite de l'astroport (sans tronquage)
        this.astroport = scene.add.sprite(x, y, 'astroport').setScale(1);
        
        // Créer le sprite de la tourelle au centre de l'astroport, décalée de 10px vers la droite
    ...
    startFireAnimation(phase) {
        ...
        switch (phase) {
            case 1: // Frame 1 à 11
                animKey = 'tourelle_new_fire1';
                startFrame = 1;
                endFrame = 11;
                break;
            case 2: // Frame 11 à 5 (rembobinage)
                animKey = 'tourelle_new_fire2';
                startFrame = 11;
                endFrame = 5;
                break;
            case 3: // Frame 5 à 2 (rembobinage final)
                animKey = 'tourelle_new_fire3';
                startFrame = 5;
                endFrame = 2;
                break;
        }
    ...
    /**
     * Tire quatre bullets en arc de cercle vers la gauche
     */
    fireBullets() {
        ...
        // Créer 4 bullets en arc de cercle vers la gauche
        const bulletCount = 4;
        const baseAngle = Math.PI; // 180° (vers la gauche)
        const arcSpread = Math.PI / 3; // 60° d'arc total
        const angleStep = arcSpread / (bulletCount - 1);
        
        for (let i = 0; i < bulletCount; i++) {
            // Calculer l'angle pour cette bullet
            ...
            // Créer la bullet
            const bullet = new TourelleBullet(this.scene, startX, startY, velocityX, velocityY);
    ...
    update(time) {
        ...
        if (!this.tourelleDestroyed && this.sprite) {
            ...
            // Maintenir le décalage de 10 pixels vers la droite pour la tourelle
...
/**
 * Classe représentant les projectiles de la tourelle
 */
class TourelleBullet {
